  ...
  useLayoutEffect(() => {
    ...
    const context = gsap.context(() => {
      cardsNodes.forEach((card, index) => {
        ...
        const initialX = profile.cardX === 0 ? 0 : direction * profile.cardX * (isSecondRow ? 0.88 : 1);
        const initialY = profile.cardY * rowDepthMultiplier + (isSecondRow ? 16 : 0);

        gsap.set(card, {
          autoAlpha: 0.08,
          x: initialX,
          y: initialY,
          rotateX: profile.cardRotateX,
          rotateY: direction * profile.cardRotateY,
          rotateZ: direction * profile.cardRotateZ,
          scale: profile.cardScale,
          transformPerspective: 1600,
          transformOrigin: isSecondRow ? "50% 16%" : "50% 84%",
          clipPath: "inset(7% 7% 7% 7% round 0.8rem)",
          filter: `blur(${profile.cardBlur}px) saturate(0.6) brightness(0.72)`,
        });

        if (image) {
          gsap.set(image, { y: profile.imageY, scale: profile.imageScale });
        }
        if (shade) {
          gsap.set(shade, { opacity: 0.76 });
        }
        if (content) {
          gsap.set(content, { autoAlpha: 0, y: profile.contentY, filter: "blur(7px)" });
        }
        if (icon) {
          gsap.set(icon, {
            autoAlpha: 0,
